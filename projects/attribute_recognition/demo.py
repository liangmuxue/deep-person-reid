...
from utils.datasets import attr_nums, description
...
def get_attr(img_path,model):
    ...
    img_data = transform_te(img_data)
    input = img_data.unsqueeze(0)
    input = input.cuda()
    prob_thre = 0.5
    orig_outputs = model(input)
    orig_outputs = orig_outputs.data.cpu().numpy()
    orig_outputs = np.squeeze(orig_outputs)
    print("orig_outputs after:{}".format(orig_outputs))
    outputs = copy.deepcopy(orig_outputs)
    outputs[outputs < prob_thre] = 0
    outputs[outputs >= prob_thre] = 1
